...
export const AWSLambdaOverviewWidget = ({ entity }: { entity: Entity }) => {
  return !isRegionInAnnotations(entity) ? (
    <WarningPanel title="AWSLambda plugin:">
      <pre>{AWS_LAMBDA_REGION_ANNOTATION}</pre>
      annotation with the lambda's region is missing on the entity.
      <br />
      Your annotations should look as following:
      <pre>{AWS_LAMBDA_ANNOTATION}: MyFunctionName</pre>
      <pre>{AWS_LAMBDA_REGION_ANNOTATION}: us-east-1</pre>
    </WarningPanel>
  ) : (
